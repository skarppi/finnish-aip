...
const parseTable = (page, offsetY, data) => {
  ...
  const separators = page.VLines
    .filter(line => line.y >= offsetY && line.w < 1)
    .sort((a, b) => a.y - b.y)

  const tableMinY = separators[0].y - separators[0].w
  const tableMaxY = separators[0].y + separators[0].l + separators[0].w

  // columns from left to right
  const cols = separators
  ...
  // loop all cells
  return pairs(rows, (rowMinY, rowMaxY) => {
    const rowBlocks = blocks.filter(block => block.y > rowMinY && block.y <= rowMaxY)

    return pairs(cols, (cellMinX, cellMaxX) =>
      rowBlocks
