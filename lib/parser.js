...
const parseTable = (page, offsetY, data) => {
  ...
  const tableMinY = vlines[0].y - vlines[0].w
  const tableMaxY = vlines[0].y + vlines[0].l// + vlines[0].w
  ...
  return transpose(pairs(cols, (cellMinX, cellMaxX) => {
    ...
    return pairs(mainRows, (rowMin, rowMax) => {
        ...
        const linesBetweenColumn = rows.filter(line => (line.x - line.w) <= cellMinX && (line.x + line.l) >= cellMaxX)

        let rowMinY = linesBetweenColumn.filter(line => line.y <= rowMin.y).reverse()[0] || rowMin
        let rowMaxY = linesBetweenColumn.filter(line => line.y >= rowMax.y)[0] || rowMax

        return colBlocks
          .filter(block => block.y > (rowMinY.y - rowMinY.w) && block.y <= (rowMaxY.y - rowMinY.w))
